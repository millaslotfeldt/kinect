...
void draw()
{
  ...
  PVector[] highestPoints = new PVector[10000];
    ...
    for(int x=0;x < context.depthWidth();x+=steps)
    {
      ...
      { 
        ...
        {  // calc the user color
          ...
          //println(userMap[index]);
          //if(index < highestPoints.length){
            int userId = userMap[index] - 1;
            // this y is smaller than the current
            if(highestPoints[userId] == null){
              highestPoints[userId] = new PVector(-10000, 0, 0);
            }
            if(realWorldPoint.y > highestPoints[userId].y ){
                // store it, overwrite it
                highestPoints[userId] = new PVector(realWorldPoint.x, realWorldPoint.y, realWorldPoint.z);
            }
          //}
        }
        else {
  ...
  for(int i = 0; i < highestPoints.length; i++){
    if(highestPoints[i] == null){
      continue;
    }
    fill(255,0,0);
//    ellipse(highestPoints[i].x, highestPoints[i].y, 50, 50);
//    ellipse(-200,-100,50,50);
    pushMatrix();
    translate(highestPoints[i].x, highestPoints[i].y, highestPoints[i].z);
    box(100);
    popMatrix();
    println(highestPoints[i]);
  }
